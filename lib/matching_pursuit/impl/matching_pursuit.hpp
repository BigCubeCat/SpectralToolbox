...
template<typename T>
matching_pursuit<T>::matrix matching_pursuit<T>::calc_correlate_matrix(matching_pursuit<T>::trace& m_trace, matching_pursuit<T>::matrix& dict){
    matching_pursuit<T>::matrix correlate_matrix(dict.rows(), m_trace.rows() + dict.cols() - 1);
    ...
    for (int i = 0; i < rows; ++i){
        for (int j = 0; j < n_left; ++j){
            correlate_matrix(i, j) = m_trace.head(j+1).dot(dict.row(i).tail(j+1));
        }
        for (int j = 0; j < trace_size - filter_size + 1; j++){
            correlate_matrix(i, n_left + j) = m_trace.segment(j, filter_size).dot(dict.row(i));
        }
        for (int j = 0; j < n_right; ++j){
            correlate_matrix(i, j + trace_size) = m_trace.tail(filter_size - (j+1)).dot(dict.row(i).head(filter_size - (j+1)));
        }
    }
...
template<typename T>
matching_pursuit<T>::trace matching_pursuit<T>::get_axis(matching_pursuit<T>::matrix& dict, int trace_size, int idx, int shift){
...
template<typename T>
matching_pursuit<T>::matrix matching_pursuit<T>::get_dict(int wavelet_length, T step_time_s){
...
std::vector<Eigen::Vector<T, Eigen::Dynamic>> matching_pursuit<T>::decompose_signal(matching_pursuit<T>::trace gather, int trace_size, T step_time_s, T max_amplitude){
    ...
    for (int i =  0; i < dict.rows(); ++i){
        spectrum.push_back(matching_pursuit<T>::trace(trace_size));
        spectrum[i].setZero();
    }
    
    for (const auto& s : S){
        matching_pursuit<T>::trace axis = get_axis(dict, trace_size, s.first, s.second);
